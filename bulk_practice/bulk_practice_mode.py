...
def run_bulk_practice_mode(all_questions, days):
    ...
    if st.session_state.bulk_index >= len(st.session_state.bulk_order):
        ...
        total = len(st.session_state.bulk_order)
        correct_count = st.session_state.bulk_correct_count
        wrong_count = total - correct_count
        accuracy = (correct_count / total) * 100 if total > 0 else 0
        
        st.success("🎉 You've completed all questions for this round!")
        
        # Add detailed round summary
        st.markdown("### 📊 Round Summary")
        st.markdown(f"- ✅ **Correct Answers:** {correct_count}")
        st.markdown(f"- ❌ **Wrong Answers:** {wrong_count}")
